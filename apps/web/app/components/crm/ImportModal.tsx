 ...
 * Multi-step wizard for importing contacts or leads from CSV
 */
...
const LEAD_FIELDS = [
  ...
  { value: 'stateProvince', label: 'State/Province', required: false },
...
const MAPPING_PATTERNS: Record<string, string[]> = {
  ...
  score: ['score', 'lead score', 'leadscore', 'rating'],
  // Address field mappings
  addressLine1: ['address', 'address line 1', 'address1', 'street', 'street address', 'registered address', 'registeredaddress'],
  addressLine2: ['address line 2', 'address2', 'suite', 'apt', 'apartment', 'unit'],
  city: ['city', 'town', 'registered address city', 'registeredaddresscity'],
  stateProvince: ['state', 'province', 'state/province', 'region', 'registered address province', 'registeredaddressprovince'],
  postalCode: ['postal code', 'postalcode', 'zip', 'zip code', 'zipcode', 'postcode', 'post code'],
...
export function ImportModal({
  ...
  const fields = entityType === 'contacts' ? CONTACT_FIELDS : LEAD_FIELDS;
  ...
  const autoMapColumn = useCallback((header: string): string | null => {
    ...
    // Special case: "Full Name" or "Name" for contacts should map to firstName
    if (entityType === 'contacts' && (normalized === 'name' || normalized === 'full name')) {
  ...
  const handlePreview = useCallback(() => {
    ...
    for (const requiredField of requiredFields) {
      if (!mappedFields.includes(requiredField)) {
        toast.error('Missing required field', { description: `Please map the "${fields.find(f => f.value === requiredField)?.label}" field` });
        return;
      }
    }
    ...
    for (let rowIdx = 0; rowIdx < Math.min(10, dataRows.length); rowIdx++) {
      ...
      for (const column of columns) {
        ...
        const field = fields.find(f => f.value === column.mappedTo);
        ...
        if (column.mappedTo === 'score' && value) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/crm/${entityType}/import`, {
        method: 'POST',
      ...
      // Show success toast
      toast.success('Import complete', { description: `Successfully imported ${result.success} ${entityType}` });
  ...
  }, [csvData, file, workspaceId, entityType, toast, onImportComplete, navigate, handleOpenChange]);
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Match your CSV columns to {entityType} fields. Fields marked with * are required.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <TableRow>
                    ...
                    {mappedColumns.map((col) => (
                      <TableHead key={col.index}>
                        {fields.find(f => f.value === col.mappedTo)?.label}
                      </TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-2">
          <DialogTitle className="text-2xl">
            Import {entityType === 'contacts' ? 'Contacts' : 'Leads'}
          </DialogTitle>
          <DialogDescription className="text-base">
            Upload a CSV file to import multiple {entityType}
          </DialogDescription>
        </DialogHeader>
        ...
        {step !== 'upload' && step !== 'complete' && (
          <div className="space-y-3 py-4">
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-3">
                <span className={step === 'mapping' ? 'font-semibold text-primary' : 'text-muted-foreground'}>
                  1. Map Columns
                </span>
                <span className="text-muted-foreground">→</span>
                <span className={step === 'preview' ? 'font-semibold text-primary' : 'text-muted-foreground'}>
                  2. Preview
                </span>
              </div>
            </div>
            <Progress value={step === 'mapping' ? 50 : 100} className="h-2" />
          </div>
        )}
            ...
            {step === 'preview' && (
              <Button onClick={handleImport} disabled={importing} size="lg">
                {importing ? (
                  ...
                  <>Import {csvData.length - 1} {entityType}</>
                )}
              </Button>
            )}
          </div>
