...
export default function LandingAlt52() {
  ...
  return (
    <div className={cn(
      ...
      theme === "dark" ? "bg-[#050505] text-slate-100" : "bg-[#ffffff] text-slate-900"
    )}>
      {/* Background Decor */}
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className={cn(
            "absolute inset-0 opacity-[0.02] dark:opacity-[0.05] pointer-events-none bg-[radial-gradient(circle_at_center,currentColor_1px,transparent_1px)] bg-[size:50px_50px]"
        )} />
      </div>

      {/* Modern High-Trust Header */}
      <nav className={cn(
        "fixed top-0 w-full z-50 transition-all duration-500 border-b",
        theme === "dark" ? "border-white/5 bg-black/40 backdrop-blur-xl" : "border-black/5 bg-white/80 backdrop-blur-xl shadow-sm"
      )}>
        <div className="container mx-auto px-12 h-24 flex items-center justify-between">
          <div className="flex items-center gap-5 group cursor-pointer text-left">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-black tracking-tighter leading-none uppercase italic">Agios<span className="text-blue-600">.</span>Prime</span>
              <span className="text-[7px] font-bold uppercase tracking-[0.6em] opacity-40 mt-1 leading-none text-left">Silicon Cape HQ</span>
            </div>
          </div>
      ...
      {/* Hero Section: Engineered for Cinematic Focus & Vertical Space */}
      <section className="relative h-screen flex flex-col items-center justify-center px-10 overflow-hidden text-center shrink-0">
        <div className="container mx-auto relative z-10 flex flex-col items-center justify-center h-full">
          <motion.div
            ...
            className="flex flex-col items-center max-w-7xl -mt-24"
          >
            <Badge className={cn(
              ...
              Advanced Lead Orchestration // v4.5.2 // Cape Town
            </Badge>
            
            <h1 className="text-6xl md:text-8xl lg:text-[10rem] xl:text-[12rem] font-black tracking-tighter mb-10 leading-[0.8] uppercase italic text-center shrink-0">
            ...
            <p className="text-xl md:text-2xl lg:text-3xl opacity-40 max-w-4xl mx-auto mb-20 font-light leading-relaxed tracking-tight shrink-0 text-center">
              Agios AI is the high-fidelity orchestration layer for South Africa's most aggressive financial lead generation pipelines.
            </p>
        ...
        {/* Tactical Hub Status Bar - Absolute Bottom with controlled opacity */}
        <div className={cn(
            "absolute bottom-10 left-0 w-full px-20 hidden lg:flex items-center justify-between font-black uppercase tracking-[0.6em] pointer-events-none transition-colors",
      ...
      {/* Section: Pipeline Performance - Resolved Cramped Layout with Max Width and Padding */}
      <section id="pipeline" className={cn(
        ...
        <div className="container mx-auto px-20 max-w-[1600px]">
            <div className="text-left mb-48 space-y-6">
                <Badge className="bg-blue-600 text-white rounded-none px-8 py-2.5 font-black text-[12px] uppercase tracking-[0.6em]">Performance Matrix</Badge>
                <h2 className="text-6xl md:text-8xl lg:text-[9rem] font-black uppercase italic tracking-tighter leading-[0.8] text-current">The Pipeline <br /> <span className="text-blue-600 not-italic">Engine.</span></h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-40 lg:gap-52 text-left">
                {[
                    { label: "Market Orchestrated", value: "R3.4B+", icon: <TrendingUp className="h-10 w-10 text-blue-600" />, desc: "Monthly pipeline volume processed across the cluster." },
                    { label: "Daily leads qualified", value: "125k+", icon: <Zap className="h-10 w-10 text-amber-500" />, desc: "Autonomous leads distributed to nodes every 24h." },
                    { label: "Compliance Score", value: "100%", icon: <ShieldCheck className="h-10 w-10 text-emerald-500" />, desc: "Regulatory hardened architecture score." },
                    { label: "Active Agent population", value: "12.8M", icon: <Fingerprint className="h-10 w-10 text-blue-600" />, desc: "Total unit count synchronized globally." }
                ].map((stat, i) => (
                    <div key={i} className="flex flex-col gap-12 text-left group">
                        ...
                        <div className="flex flex-col gap-6 text-left">
                            <span className="text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter italic leading-none text-current transition-colors group-hover:text-blue-600">{stat.value}</span>
                            <div className="space-y-4 pt-8 border-t border-current/10">
                                <span className="text-[12px] font-black uppercase tracking-[0.6em] opacity-40 leading-none block text-left">{stat.label}</span>
      ...
      <section id="logic" className="py-72 px-12 relative overflow-hidden text-left shrink-0">
        <div className="container mx-auto max-w-[1500px]">
            <div className="flex flex-col lg:flex-row items-center gap-52 text-left">
                <div className="flex-1 space-y-28 text-left">
                    <div className="space-y-12 text-left">
                        ...
                        <h2 className="text-6xl md:text-8xl lg:text-[11rem] font-black uppercase tracking-tighter leading-none italic text-current text-left">
                        ...
                        <p className="text-2xl md:text-3xl opacity-50 font-light leading-relaxed max-w-4xl tracking-tight text-left">
                            Deploy specialized autonomous units trained on localized financial data. We map the digital landscape with surgical precision.
                        </p>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-20 text-left">
                        {[
                            { title: "Market Sourcing", desc: "Autonomous agents scan market signals across 50M+ South African data points." },
                            { title: "Neural Scoring", desc: "Proprietary qualification models built on localized financial readiness and FSR scoring." },
                        ].map((item, i) => (
                            <div key={i} className="space-y-6 group text-left">
                                <h3 className="text-lg font-black uppercase tracking-widest text-blue-600 flex items-center gap-6 italic group-hover:text-blue-500 transition-colors text-left leading-none">
                                    <span className="h-px w-12 bg-blue-600/30 group-hover:w-20 transition-all" /> 
                                    {item.title}
                                </h3>
                                <p className="text-base opacity-40 font-light leading-relaxed text-left">{item.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="w-full lg:w-[45%] relative shrink-0">
                    <div className={cn(
                        ...
                        theme === "dark" ? "border-white/10 bg-black" : "border-black/5 bg-white shadow-black/5"
                    )}>
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center">
                            ...
                            <div className="text-center space-y-6 relative">
                                ...
                                <div className="text-5xl lg:text-7xl font-black italic uppercase tracking-tighter leading-none relative z-10 text-current leading-none">Synchronized</div>
                        ...
                        <div className="flex items-center justify-between border-t border-current/10 pt-10">
                            <div className="flex flex-col gap-2 text-left">
                                ...
                                <span className="text-3xl font-black italic leading-none text-current">99.99%</span>
                            </div>
      ...
      <section id="compliance" className={cn(
        ...
        <div className="container mx-auto px-10 max-w-[1400px] text-center space-y-32 text-left md:text-center">
            <div className="flex justify-center">
                <div className="h-32 w-32 border-2 border-blue-600 flex items-center justify-center rounded-full transform hover:scale-110 transition-all duration-1000 cursor-pointer group shadow-2xl">
                    <Lock className="h-14 w-14 text-blue-600 group-hover:scale-110 transition-transform" />
                </div>
            </div>
      ...
      <footer className={cn(
        ...
        <div className="container mx-auto px-20">
            ...
            <div className="py-72 text-center relative mb-48 flex flex-col items-center">
                ...
                <h2 className="text-8xl md:text-[16rem] lg:text-[22rem] font-black tracking-tighter uppercase leading-[0.7] mb-24 italic relative z-10 text-center text-current opacity-[0.02] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap select-none pointer-events-none">
                    AGIOS PRIME
                </h2>
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-20 relative z-10 mx-auto w-full max-w-6xl">
            ...
            <div className="flex flex-col lg:flex-row items-start justify-between gap-40 mb-40 text-left pt-32 border-t border-current/5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-32 text-left flex-1 text-current">
                    <div className="space-y-12 text-left">
                        ...
                        <ul className="space-y-8 text-[11px] font-bold uppercase tracking-widest opacity-40 text-left text-left">
            ...
            <div className="pt-20 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-16 opacity-20 text-[12px] font-black uppercase tracking-[0.8em] text-left text-current leading-none">
