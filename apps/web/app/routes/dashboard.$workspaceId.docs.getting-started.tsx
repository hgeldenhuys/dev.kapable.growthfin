...
import {
  ...
  ChevronRight, CheckCircle2, ArrowRight, Keyboard, HelpCircle,
} from 'lucide-react';
    ...
    <div className="space-y-6 pb-12 max-w-4xl">
      ...
      <StepCard step={10} title="Using the AI Agent" icon={<Bot className="h-5 w-5 text-muted-foreground" />}>
        ...
        <div className="rounded-lg border p-3 bg-muted/30 mt-2">
          <p className="text-sm">
            <strong>Note:</strong> The AI Agent requires an OpenRouter API key configured in{' '}
            <Link to={`/dashboard/${workspaceId}/settings`} className="text-primary underline">Settings</Link>.
      ...
      <div className="text-center py-6 text-muted-foreground">
        ...
        <p>Start by{' '}
          <Link to={`/dashboard/${workspaceId}/crm/leads/new`} className="text-primary underline">creating a lead</Link>
          {' '}and exploring the dashboard.
        </p>
      </div>
    </div>
  );
